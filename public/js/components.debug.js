...
//        $frame.css({width: screenSize.width * zoom});
      $('.debug-scroll-pane-thumb.horizontal').css({ top: screenSize.y + $scrollPane.height() * 0.5 - debugScrollPaneThumbSize * 0.5, left: screenSize.x + $scrollPane.width() });
      $('.debug-scroll-pane-thumb.vertical').css({ top: screenSize.y + $scrollPane.height(), left: screenSize.x + $scrollPane.width() * 0.5 - debugScrollPaneThumbSize * 0.5 });
    ...
    function readURL(input) {
      ...
      if (fileName) {
        ...
        if (fileName === defaultFile) {
          $('.debug-image-overlay').css({ 'background-image': 'url("' + defaultPath + defaultFile + '")', 'background-repeat': 'repeat' });
        } else {
          $('.debug-image-overlay').css({ background: 'url("' +
...
//                '100%';
      ...
      if (pageLock) {
        pageLockDimensions = { w: window.outerWidth || $(window).width(), h: window.outerHeight || $(window).height(), t: $(window).scrollTop(), l: $(window).scrollLeft() };
...
//      console.log(transStr)
      $('.debug-image-overlay img').css({ 'transform-origin': 'top ' + SETTINGS.overlayImage.alignment, transform: transStr });
...
//        $('#debug-thingy-pipetteInfo').text(e.pageX +" "+ e.pageY +" "+ e.offsetX +" "+ e.offsetY +" "+ el.toString());
    ...
    function measureRectBorderMouseDownHandler(e) {
      mouseInfoObject.mousePosition = { x: Math.round(e.pageX / internalZoom), y: Math.round(e.pageY / internalZoom) };
...
//      console.log($('body .mouse-move-event-target-dummy'))
    ...
    function updateMeasureRect() {
      ...
      $('.measure-rect .measure-rect-border').css({ left: measureRectInternal.left, top: measureRectInternal.top, width: measureRect.width, height: measureRect.height });
      ...
      $('.measure-rect .measure-rect-thumb.top.left').css({ left: measureRectInternal.left - thumbHalfSize + thumbOffsetH, top: measureRectInternal.top - thumbHalfSize + thumbOffsetV });
      $('.measure-rect .measure-rect-thumb.top.right').css({ left: measureRectInternal.right - thumbHalfSize - thumbOffsetH, top: measureRectInternal.top - thumbHalfSize + thumbOffsetV });
      $('.measure-rect .measure-rect-thumb.bottom.left').css({ left: measureRectInternal.left - thumbHalfSize + thumbOffsetH, top: measureRectInternal.bottom - thumbHalfSize - thumbOffsetV });
      $('.measure-rect .measure-rect-thumb.bottom.right').css({ left: measureRectInternal.right - thumbHalfSize - thumbOffsetH, top: measureRectInternal.bottom - thumbHalfSize - thumbOffsetV });

      $('.measure-rect .measure-rect-background.top').css({ top: 0, left: 0, right: 0, bottom: 'auto', height: measureRectInternal.top });
      $('.measure-rect .measure-rect-background.bottom').css({ top: measureRectInternal.bottom, left: 0, right: 0, bottom: 0 });
      $('.measure-rect .measure-rect-background.left').css({ top: measureRectInternal.top, left: 0, right: 'auto', bottom: 'auto', height: measureRect.height, width: measureRectInternal.left });
      $('.measure-rect .measure-rect-background.right').css({ top: measureRectInternal.top, left: measureRectInternal.right, right: 0, bottom: 'auto', height: measureRect.height });
